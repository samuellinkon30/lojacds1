...
Checkout.prototype = {
    ...
    loadingbox: function() {
        $("oscheckout-review").update('<div class="ajax-load">&nbsp;</div>')
    },
    loadReview: function(step) {
        var request = new Ajax.Request(this.reviewUrl, {
            method: 'post',
            parameters: Form.serialize('oscheckout-form'),
            onLoading: this.loadingbox.bind(this),
            onComplete: this.onComplete,
            onSuccess: function(transport) {
                if (transport.status == 200) {
                    $("oscheckout-review").update(transport.responseText)
                }
            },
...
Payment.prototype = {
    ...
    loadReview: function(step) {
        var updater = new Ajax.Updater('oscheckout-review', this.reviewUrl, {
            method: 'get',
            onLoading: this.loadingbox.bind(this),
    ...
    switchMethod: function(method) {
        ...
        this.loadReview('payment');
    },

    loadingboxPayment: function() {
        $("ajax-payment-methods").update('<div class="ajax-load">&nbsp;</div>')
    },
    reloadPaymentBlockNew: function(step) {
        var updater = new Ajax.Updater('ajax-payment-methods', this.paymentUrl, {
            method: 'get',
            onLoading: this.loadingboxPayment.bind(this),
            onFailure: this.ajaxFailure.bind(this),
            onSuccess: function() {},
            onComplete: function() {
               payment.switchMethodNew(paymentMethod);
            }
        });
    },
    switchMethodNew: function(method){
        $('p_method_' + this.currentMethod).checked =true;
        $('payment_form_' + this.currentMethod).style.display = '';
    },
...
Review.prototype = {
    ...
    loadingbox: function() {
        $("review-require").update(' <div class="please-wait-loading">&nbsp;</div>')
        var form = $('review-btn');
        form.disabled = 'true';
    },
    save: function() {
...
Shipping.prototype = {
    ...
    loadReview: function() {
        // payment.reloadPaymentBlockNew("payment");
        //clear credit card
        jQuery('#cardNumber').val('');
        jQuery('#cardNumber').css('background', 'none');
        jQuery('#installments').find('option').not(':first').remove();  
        //add amount for installment again
        
        var updater = new Ajax.Updater('oscheckout-review', this.reloadUrl, {
            method: 'post',
            onLoading: this.loadingbox.bind(this),
            parameters: Form.serialize(this.form)
        });
        setTimeout(function(){
            jQuery.ajax({
            url:"/oscheckout/index/getgrandtotal",  
            success:function(data) {
                var data = jQuery.parseJSON(data);
                console.log(data.grandtotal); 
                jQuery('#amount').val(data.grandtotal);
            }
        });
        }, 2000);
...
function changeCheckoutMethod() {
    if ($('login:guest').checked) {
        Element.hide('passowrd-container');
        $('register_customer').value = '';
    } else {
        Element.show('passowrd-container');
        $('register_customer').value = 'register';
    }
}
