...
/**
 * Type admin edit tab attributes block
 * @category    Ave
 * @package     Ave_SizeChart
 * @author      averun <dev@averun.com>
*/
class Ave_SizeChart_Block_Adminhtml_Type_Edit_Tab_Attributes extends Mage_Adminhtml_Block_Widget_Form
{
    /**
    ...
    protected function _prepareForm()
    {
        ...
        $form->setDataObject(Mage::registry('current_type'));
        $fieldset = $form->addFieldset(
            'info',
            array(
                'legend' => Mage::helper('ave_sizechart')->__('Type Information'),
                'class' => 'fieldset-wide',
            )
        );
        $attributes = $this->getAttributes();
        foreach ($attributes as $attribute) {
            $attribute->setEntity(Mage::getResourceModel('ave_sizechart/type'));
        }

        $this->_setFieldset($attributes, $fieldset, array());
        $formValues = Mage::registry('current_type')->getData();
    ...
    protected function _getAdditionalElementTypes()
    {
        return array(
            'file'     => Mage::getConfig()->getBlockClassName(
                'ave_sizechart/adminhtml_type_helper_file'
            ),
            'image'    => Mage::getConfig()->getBlockClassName(
                'ave_sizechart/adminhtml_type_helper_image'
            ),
