 ...
 */
class Codazon_Flexibletheme_Adminhtml_Flexibletheme_ConfigController extends Mage_Adminhtml_Controller_Action
{
    
    const THEME_MODEL = 'flexibletheme/theme';
    /**
    ...
    {
        $this->_title($this->__('Theme List'))->_title($this->__('Theme List'));
        
        $this->loadLayout();
        $this->getLayout()->getBlock("head")->setCanLoadExtJs(true);
        $this->_setActiveMenu('flexibletheme/config');
        $this->_addBreadcrumb($this->__("Flexible Theme"), $this->__("Flexible Theme"));
        $this->_addBreadcrumb($this->__("Theme List"), $this->__("Theme List"));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        if (!$themeId) {
            $session->addError($this->__('Theme ID is empty'));
            return $this->_redirect('*/*/', array('_current' => true, 'section' => false));
        }
        $themeModel = Mage::getModel(self::THEME_MODEL)->load($themeId);
        if (!$themeModel->getId()) {
            $session->addError($this->__('Theme with ID %s hasn\'t been assigned', $themeId));
            return $this->_redirect('*/*/', array('_current' => true, 'section' => false, 'theme_id' => false));
        }
        Mage::register("current_theme", $themeModel);
        
        Mage::getSingleton('flexibletheme/config_data')
            ->setSection($current)
            ->setWebsite($website)
            ->setStore($store)
            ->setThemeId($themeId);

        $configFields = Mage::getSingleton('flexibletheme/config');
        ...
        if (!$hasChildren && $current) {
            $this->_redirect('*/*/', array('website' => $website, 'store' => $store, 'theme_id' => $themeId));
        }
        ...
        $this->getLayout()->getBlock('left')
            ->append($this->getLayout()->createBlock('flexibletheme/adminhtml_system_config_tabs')->initTabs());

        if ($this->_isSectionAllowedFlag) {
            $this->_addContent($this->getLayout()->createBlock('flexibletheme/adminhtml_system_config_edit')->initForm());
    ...
    public function saveAction()
    {
        ...
        //a($this->getRequest()->getParams()); die();
        $session = Mage::getSingleton('adminhtml/session');
        ...
        try {
            ...
            Mage::helper('flexibletheme')->getThemeConfig()->reinit();
            Mage::dispatchEvent('admin_theme_config_section_save_after', array(
                'website'   => $website,
                'store'     => $store,
                'section'   => $section,
                'theme_id'  => $themeId
            ...
            Mage::dispatchEvent("admin_theme_config_changed_section_{$section}",
        ...
        $this->_redirect('*/*/edit', array('_current' => array('section', 'website', 'store', 'theme_id')));
    }
    ...
    {
        return true; //Mage::getSingleton('admin/session')->isAllowed('system/config');
    }
    ...
    public function activatethemeAction()
    {
        ...
        try {
            ...
            if (!$themeModel->getIsActive()) {
                $session->addError($this->__('This theme is not active'));
                return $this->_redirect('*/*/', array('_current' => true, 'section' => false, 'theme_id' => false));
            }
            $package = $themeModel->getData('theme_package');
            $template = $themeModel->getData('theme_template');
            $themeTitle = $themeModel->getData('theme_title');
            ...
            $this->getRequest()->setPost('groups', $groups);
            ...
            /* Change home page */
            if (Mage::getModel('cms/page')->load('codazon-home-page', 'identifier')->getId()) {
                $section = 'web';
                $groups = array(
                    'default'   => array(
                        'fields' => array(
            ...
            $session->addSuccess(Mage::helper('adminhtml')->__("The theme '%s' have been activated.", $themeTitle));
        } catch (Mage_Core_Exception $e) {
            foreach (explode("\n", $e->getMessage()) as $message) {
                $session->addError($message);
            }
