...
class Codazon_Flexibletheme_Model_Config_Data extends Varien_Object
{
    /**
    ...
    public function save()
    {
        ...
        Mage::dispatchEvent('model_theme_config_data_save_before', array('object' => $this));
        ...
        $sections = Mage::getModel('flexibletheme/config')->getSections();
        ...
        foreach ($groups as $group => $groupData) {
            ...
            foreach ($groupData['fields'] as $field => $fieldData) {
                ...
                if (!$backendClass) {
                    $backendClass = 'flexibletheme/themeconfig_data';
                }

                $dataObject = Mage::getModel($backendClass);
                if (!$dataObject instanceof Codazon_Flexibletheme_Model_Themeconfig_Data) {
    ...
    {
        $configDataCollection = Mage::getModel('flexibletheme/themeconfig_data')
            ->getCollection()
            ->addThemeIdFilter($this->getThemeId())
    ...
    public function getConfigRoot()
    {
        if (is_null($this->_configRoot)) {
            $this->load();
            $this->_configRoot = Mage::helper('flexibletheme')->getThemeConfig()->getNode(null, $this->getScope(), $this->getScopeCode());
        }
