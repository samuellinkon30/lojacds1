...
class Codazon_Quickviewpro_Block_Adminhtml_Url extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        $this->setElement($element);
        $output = '<script type="text/javascript">//<![CDATA[' . "\n";
        $output .= '    var xml_form_template_'.$this->getElement()->getId().' = \'' . str_replace("'", "\'", $this->_getRowEditHtml()) .'\';' . "\n";
        $output .= '//]]></script>' . "\n";
    ...
    protected function _getHeaderHtml()
    {
        ...
        $output .= Mage::helper('quickviewpro')->__('ID/Class selector of block to be updated');
        $output .= '</th>';
        $output .= '<th style="padding: 2px; text-align: center;">';
        $output .= Mage::helper('quickviewpro')->__('Layout Update Block name (should be same as in XML)');
        $output .= '</th>';
        $output .= '<th>&nbsp;</th>';
        $output .= '</tr>';
        return $output;
    ...
    protected function _getAddButtonHtml()
    {
        return $this->getLayout()->createBlock('adminhtml/widget_button')
            ...
            ->setOnClick("Element.insert($(this).up('tr'), {before: xml_form_template_".$this->getElement()->getId()."})")
            ->toHtml();
    }
